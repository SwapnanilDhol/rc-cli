...
func runChartsOverview(cmd *cobra.Command, args []string) error {
	...
	path := fmt.Sprintf("/developers/me/charts_v2/overview?app_uuid=%s&sandbox_mode=%t", c.ProjectID, sandbox)
	if appUUID != "" {
...
func runChartsTrials(cmd *cobra.Command, args []string) error {
	...
	path := fmt.Sprintf("/developers/me/charts_v2/trials?app_uuid=%s&sandbox_mode=%t&resolution=%d", c.ProjectID, sandbox, resolution)
	if appUUID != "" {
...
func runChartsTransactions(cmd *cobra.Command, args []string) error {
	...
	path := fmt.Sprintf("/developers/me/charts_v2/transactions?app_uuid=%s&sandbox_mode=%t&resolution=%d", c.ProjectID, sandbox, resolution)
	if appUUID != "" {
...
func runChartsRevenue(cmd *cobra.Command, args []string) error {
	...
	path := fmt.Sprintf("/developers/me/charts_v2/revenue?app_uuid=%s&sandbox_mode=%t&resolution=%d", c.ProjectID, sandbox, resolution)
	if appUUID != "" {
